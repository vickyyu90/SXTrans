...
default_cfgs = {
    ...
    'vit_base_patch16_384': _cfg(
        ...
        input_size=(3, 384, 384), mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), crop_pct=1.0),
...
class Attention(nn.Module):
    def __init__(self, dim=2048, num_heads=8, num_patches = 197, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., vis=False):
        super().__init__()
        self.vis = vis
        ...
        self.scale = nn.Parameter(torch.full((1, num_heads, num_patches), 0.05), requires_grad=True)
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qk(x).reshape(B, N, -1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k = qkv[0], qkv[1]
        v = self.v(x).reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)

        attn = (q * k) * self.scale.unsqueeze(-1)
        attn = self.attn_drop(attn)

        attn = (v.reshape(B, self.num_heads,N, C // self.num_heads, -1) * attn.unsqueeze(-1)).sum(dim=-1)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, num_patches, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            dim, num_heads=num_heads, num_patches = num_patches, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, vis=vis)
...
class VisionTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=14, embed_dim=2048, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, num_patches=num_patches+1,mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
